...
const NOTES = {
  ...
  "E": 4, "e": 4, "d##": 4, "D##": 4, "fb": 4, "Fb": 4, "FB": 4,
...
export function parsePitch(pitch: string): Note {
  const pitchClass = pitch.match(/[a-gA-G][b|#]{0,2}(?![a-zA-Z])/)?.[0] as PitchClass;
 ...
 * @return midi number
 */
function octaveNoteToMidi(octave = 4, interval: number) {
  return (octave * 12) + interval + 12;
}

export function noteInfo({ pitchClass, octave }: Note) {
...
export function noteToMidi(note: string | Note) {
  if (isString(note)) {
...
export function noteToHz(note: string | Note) {
